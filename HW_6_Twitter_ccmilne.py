...
def test_oauth():
    ...
    authentication_state = requests.get(url, auth=auth).json() #replace json with .status_code to print out the status code
...
def make_request_with_cache(baseurl, hashtag, count):
    ...
    temporary_params = {'q': hashtag_lower, 'count': count}

    request_key = construct_unique_key(baseurl, params=temporary_params)
    if request_key in CACHE_DICT.keys():
        print(f"fetching cached data")
        return CACHE_DICT[request_key]
    else:
        print(f"making new request")
        CACHE_DICT[request_key] = make_request(baseurl, params=temporary_params)
...
def find_most_common_cooccurring_hashtag(tweet_data, hashtag_to_ignore):
    ...
    for tweet in tweets:
        for hashtag in tweet['entities']['hashtags']:
            if hashtag['text'].lower() not in hashtag_dict:
                hashtag_dict[hashtag['text'].lower()] = 1
            else:
                hashtag_dict[hashtag['text'].lower()] += 1

    removed_hashtag_lower = hashtag_dict.pop(hashtag_to_ignore[1:].lower())
    print(hashtag_dict)

    top_hashtag_as_string = max(hashtag_dict, key=hashtag_dict.get)
    top_hashtag = top_hashtag_as_string.lower()
...
if __name__ == "__main__":
    ...
    print("\nThe most commonly cooccurring hashtag with {} is {}.".format(hashtag, most_common_cooccurring_hashtag))
